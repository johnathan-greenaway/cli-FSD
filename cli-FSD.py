...
parser.add_argument("-a", "--autopilot", action="store_true", default='False',help="Turn autopilot mode on")
...
def process_input_in_autopilot_mode(user_input):
...
if __name__ == "__main__":
    autopilot_mode = args.autopilot or (args.autopilot_choice == 'on')
    cleanup_previous_assembled_scripts()
    print_instructions

    if args.query:
        process_input_in_autopilot_mode(args.query)
    else:
        ...
        while True:
            if command_mode:
                ...
                if user_input.upper() == 'CMD':
                    ...
                    process_input_in_autopilot_mode(user_input)
                else:
